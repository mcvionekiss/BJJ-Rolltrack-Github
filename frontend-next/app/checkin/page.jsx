...
function Checkin() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      console.log(
        `🔶 Making API request to check_student with email: ${email}`
      );
      const startTime = performance.now();
      const response = await axios.post(
        "http://192.168.2.1:8000/api/check_student/",
        { email }
      );
      ...
      if (response.data.exists) {
        ...
        router.push("/available-classes", { state: { email } });
      } else {
      ...
      console.error("🔴 Student not found:", {
        email,
        errorMessage:
          error.response?.data?.message || "Error checking student.",
      ...
      console.log(
        "🔶 Error state updated with message: Email not found. Please try again or contact an instructor."
      );
    }
  };
